...
const Paginator = React.memo(({
    ...
    return <div className={portionNumber === 1 ? styles.pages + ' ' + styles.pagesStart : styles.pages}>
        {portionNumber > 1 && <div className={styles.btn} onClick={(e) => {
            e.preventDefault()
        ...
        {pages
            ...
            .map(p => {
                return <span className={currentPage === p ? styles.active + ' ' + styles.page : styles.page}
                             onClick={(e) => {
                                 onPageChanged(p)
                             }}>{p}</span>
            })}
        {portionsCount > portionNumber && <div className={styles.btn} onClick={(e) => {
            e.preventDefault()
            setPortion(portionNumber += 1)
        }}>
            <a href=''>next</a>
